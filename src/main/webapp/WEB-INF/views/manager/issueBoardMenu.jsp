				...
				<c:choose>
					...
					<c:otherwise>
						...
						<c:forEach items="${issueBoardList}" var="issueBoard">
							<tbody>
								<tr>
									<th scope="row">${issueBoard.issueNum}</th>
									<td>${issueBoard.issueTitle}</td>
									<td>${issueBoard.name}</td>
									<td>
										<button class="smBtn" type="button" onclick="return updateConfirm(${issueBoard.issueNum})">수정</button>
										<button class="smBtn" type="button" onclick="return deleteConfirm(${issueBoard.issueNum})">삭제</button>
									</td>
								</tr>
							</tbody>
						</c:forEach>
						...
						<c:if test="${(sectionPage.section*100)<totalCnt }"> <!-- 다음 섹션이 존재하는가 => '>>' O -->
							<c:forEach var="page" begin="1" end="10" step="1"> <!-- 번호 매기기 -->
								<c:if test="${sectionPage.section>1 && page==1}">
									<li class="page-item">
								      <a class="page-link" aria-label="Previous" href="/animalCommunity/boardManagement/issue/${sectionPage.section-1}/${10}">
								        <span aria-hidden="true">&laquo;</span>
								      </a>
								    </li>
								</c:if>
								<li class="page-item">
									<a class="page-link" href="/animalCommunity/boardManagement/issue/${sectionPage.section}/${page}">
								...
								<c:if test="${page==10}">
									<li class="page-item">
								      <a class="page-link" href="/animalCommunity/boardManagement/issue/${sectionPage.section+1}/${1}" aria-label="Next">
